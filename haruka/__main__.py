...
def send_start(bot, update):
    ...
    text = "Hey there! My name is Punisher - I'm here to help you manage your groups!\n\
...
def control_panel(bot, update):
    ...
    if M_match:
        ...
        if mod_match:
            ...
            text = f"You has the following settings for the *{CHAT_SETTINGS[module].__mod_name__}* module:\n\n{R[0]}"
...
def send_settings(chat_id, user_id, update, user=False):
    if user:
        if USER_SETTINGS:
            settings = "\n\n".join(
                f"*{mod.__mod_name__}*:\n{mod.__user_settings__(Bot, update, user_id)}"
